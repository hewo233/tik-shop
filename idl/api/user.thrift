...
struct RegisterRequest {
  1: string username (api.body = "username", api.form = "username", api.vd = "(len($) > 0 && len($) < 128)")
...
struct UpdateUserRequest {
  1: i64 id (api.path = "id", api.vd = "$>0")
  2: string name (api.body = "username", api.form = "username", api.vd = "(len($) > 0 && len($) < 128)")
...
service UserService {
  ...
  base.MessageResponse register(1: RegisterRequest req) (api.post="/api/auth/register")

  UserResponse getUserInfoByID(1: GetUserInfoByIDRequest req) (api.get="/api/user/:id")
